...
class ScheduleHistoryPdf < Prawn::Document
  ...
  def header
    text "List of Deleted Schedules", size: 14, style: :bold, align: :center
    move_down 20
  ...
  def data_rows
    ...
    [["#", "Description", "Subject", "Day", "Start Time", "End Time", "Professor Name", "Room", "School Year", "Deletion Date"]] +
      @schedules.each_with_index.map do |schedule, index|
        ...
        [
          ...
          schedule.deleted_at.strftime('%Y-%m-%d')
        ]
      end
  end
end
