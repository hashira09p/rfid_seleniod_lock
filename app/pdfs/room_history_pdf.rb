...
class RoomHistoryPdf < Prawn::Document
  ...
  def header
    text "List of Deleted Rooms", size: 20, style: :bold, align: :center
    move_down 20
  ...
  def data_rows
    ...
    [["#", "Room Number", "Status", "Deletion Date"]] +
      @rooms.each_with_index.map do |room, index|
        [
          index + 1,
          room.room_number,
          room.room_status,
          room.deleted_at.strftime('%Y-%m-%d')
        ]
      end
  end
end
