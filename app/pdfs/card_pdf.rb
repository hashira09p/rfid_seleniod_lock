...
class CardPdf < Prawn::Document
  ...
  def header
    text "List of Cards", size: 20, style: :bold, align: :center
    move_down 20
  ...
  def data_rows
    ...
    [["#", "Card Number", "Name", "Card Type", "Registered Date", "Status"]] +
      @cards.each_with_index.map do |card, index|
        [
          ...
          card.created_at.in_time_zone('Asia/Manila').strftime('%Y-%m-%d'),
          card.status
        ]
      end
  end
